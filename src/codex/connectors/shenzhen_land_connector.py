...
class ShenzhenLandConnector:
    """
    深圳土地市场连接器
    
    数据来源：深圳市规划和自然资源局
    URL: https://pnr.sz.gov.cn/ywzy/tdjygs/
    """
    
    BASE_URL = "https://pnr.sz.gov.cn/ywzy/tdjygs"
    ...
    def _parse_list_page(self, html: str) -> List[Dict[str, Any]]:
        ...
        # 正则模式：匹配 <li class="list-group-item">
        # 格式：<li class="list-group-item"><span>日期</span><a href="...">标题</a></li>
        pattern = r'<li[^>]*list-group-item[^>]*>\s*<span[^>]*>([^<]+)</span>\s*<a[^>]*href="([^"]+)"[^>]*>([^<]+)</a>'
        ...
        for date, href, title in matches:
            ...
            if href.startswith('http'):
                ...
                full_url = 'https://pnr.sz.gov.cn' + href
    ...
    def normalize_land_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'category': 'land',
            'title': raw_data.get('title') or '深圳土地交易公告',
            'content': f"公告：{raw_data.get('title', '')}，日期：{raw_data.get('date', '')}",
            'city': '深圳',
            'date': raw_data.get('date') or '',
            'source': raw_data.get('source') or '深圳市规划和自然资源局',
...
def test_shenzhen_land_connector():
    """测试深圳土地市场连接器"""
    print("🧪 测试深圳土地市场连接器")
    print("=" * 60)
...
if __name__ == '__main__':
    result = test_shenzhen_land_connector()
