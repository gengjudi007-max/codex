...
def _policy_semantics(payload: Dict[str, Any]) -> Dict[str, Any]:
    current = payload.get("current") or payload.get("text") or payload.get("message")
    previous = payload.get("previous") or ""
    return {
        "mode": "policy_semantics",
...
def _fetch_sources(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "mode": "fetch_sources",
        "result": fetch_source_dicts(payload.get("sources", [])),
    }


def _parse_documents(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "mode": "parse_documents",
        "result": parse_documents(payload.get("paths", [])),
    }


def _import_terminal(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "mode": "import_terminal",
        "result": import_terminal_file(str(payload.get("path", ""))),
    }


def _ifind_query(payload: Dict[str, Any]) -> Dict[str, Any]:
    result = run_ifind_query(str(payload.get("ifind", "")))
    return {
        "mode": "ifind_query",
        "result": {
            "raw": result,
...
def _search_store(payload: Dict[str, Any]) -> Dict[str, Any]:
    path = str(payload.get("path") or payload.get("store") or "")
    ...
    return {
        "mode": "search_store",
        "result": {
            "search": search_jsonl(path, query=query, limit=limit),
